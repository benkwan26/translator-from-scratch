...
class DecoderLayer(nn.Module):
    def __init__(self, d_model, ffn_hidden, num_heads, drop_prob):
        ...
        self.layer_norm1 = LayerNormalization(parametres_shape=[d_model])
        self.dropout1 = nn.Dropout(drop_prob)

        self.encoder_decoder_attention = MultiHeadCrossAttention(d_model, num_heads)
        self.layer_norm2 = LayerNormalization(parametres_shape=[d_model])
        self.dropout2 = nn.Dropout(drop_prob)
...
class Transformer(nn.Module):
    def __init__(self, d_model, ffn_hidden, num_heads, drop_prob, num_layers, max_sequence_length, kn_vocab_size, english_to_index, french_to_index, START_TOKEN, END_TOKEN, PADDING_TOKEN):
        super().__init__()
    ...
    def forward(self, x, y, encoder_self_attention_mask=None, decoder_self_attention_mask=None, decoder_cross_attention_mask=None, enc_start_token=False, enc_end_token=False, dec_start_token=False, dec_end_token=False):
        ...
        output = self.decoder(x, y, decoder_self_attention_mask, decoder_cross_attention_mask, START_TOKEN=dec_start_token, END_TOKEN=dec_end_token)
